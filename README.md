...
SwiftUI desktop app (macOS) for managing invoices for a Slovenian sole trader.
...
- **Issuing** — issuing a draft assigns the next sequential number (`2026-001`) and locks
  the invoice
- **PDF export** — A4 invoice with the mandatory Slovenian fields, paginated for long
  invoices; drafts carry a DRAFT watermark so one cannot be mistaken for a real invoice
- **Preview** — the invoice editor shows the PDF live in a trailing inspector, re-rendered
...
- **Year overview** — every issued invoice of one calendar year in one table, with a year
  picker and the TOTAL line, and **XLSX export** writing the same table as a real Excel
  workbook — dates and amounts as values, not text, so the accountant can sort and sum
  them. The sheet is written for the accountant, not for the app: its headers are
  Slovenian in every interface language, like the printed invoice (see *English /
  Slovenian* below)
- **Invoice template** — logo, tagline, signature and the three sentences (intro, payment
  instruction, closing) live in Settings › Invoice template. Sentences take placeholders
  such as `{MONTH}`, `{year}`, `{iban}`, `{reference}`; see `Core/InvoiceTemplate.swift`.
  Templates saved before the placeholders were renamed still resolve — the old spellings
  are mapped in `InvoiceTemplate.legacyTokens`
- **Settings** — your own business details (name, address, bank account, tax status,
  IBAN, default payment term, footer note), in the sidebar and under ⌘,
...
- **English / Slovenian** — the interface follows the Mac's language, or the picker in
  Settings › Language. Every string lives in `Resources/Localizable.xcstrings` with
  **English as the source language**: write the English text as the key in code, and
  Slovenian is the translation hanging off it. Slovenian appears nowhere else — not in
  code, comments, tests or this file. The printed invoice, the .xlsx sheet, the exported
  file names, the template defaults and the sample invoice are Slovenian documents
  whatever language the interface runs in: their keys are English like any other, but
  they are resolved through `Core/DocumentText.swift`, which reads the Slovenian
  localization directly. A new label on the PDF page or in `Core/YearOverviewXLSX.swift`
  goes through `DocumentText.string`, gets its Slovenian translation in the catalog, and
  is added to the key list in `LocalizationTests`, which fails when a document key would
  fall back to English. Counts go through the catalog's plural rules rather than an `if`:
  Slovenian needs `one` / `two` / `few` / `other` where English needs two, and
  ...
  because the status of one invoice and the year's total received decline differently in
  Slovenian, and both are "Paid" in English
...
Not built yet: printing, e-invoices, expenses, search and filtering.
...
sheet a year overview fills is laid out in `Core/YearOverviewXLSX.swift`. The TOTAL
...
- **Issued invoices are immutable.** `InvoiceStatus.isEditable` is true only for drafts;
  the editor disables itself otherwise. Corrections belong in a cancellation or a credit
  note.
- **Service date** is stored separately from the issue date, since both are mandatory
  and frequently differ.
- **Not VAT registered.** `VatRate.exempt` charges 0 % and carries the Article 94(1)
  ZDDV-1 exemption clause, printed under the totals.
...
- **Cancellation is a v0 simplification.** Today "Cancel invoice" flips the invoice to
  `.cancelled`. Properly, a cancellation is its own numbered document (a cancellation
  invoice or a credit note) that references the original — worth fixing before this is
  used for real bookkeeping.
...
- **Flat-rate expenses** — costs are recognised as a fixed share of revenue, so the app
  does not need to track actual costs. Only revenue matters.
- **Not VAT registered** — invoices charge no VAT. The VAT rate picker and the VAT recap
  stay hidden, and every invoice carries the Article 94 ZDDV-1 exemption clause.
- **Bank transfer only** — no cash, so the fiscal verification of receipts (the tax
  office's cash-register reporting) does not apply and is permanently out of scope.
...
1. **Any public-sector clients?** Those require e-invoices in eSLOG 2.0 via the public
   payments administration (UJP) — a real chunk of work, worth knowing up front.
